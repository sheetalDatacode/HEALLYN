...
exports.createSupportTicket = asyncHandler(async (req, res) => {
  ...
  // Get nurse data for notifications
  ...
  try {
    // Send confirmation email to nurse
    if (nurse) {
      await sendSupportTicketNotification({
        ...
        userType: 'nurse',
        isResponse: false,
      }).catch((error) => console.error('Error sending support ticket email to nurse:', error));
      
      // Create in-app notification for nurse
...
// GET /api/nurses/support/history
