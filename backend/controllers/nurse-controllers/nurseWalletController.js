...
const { ROLES, getCommissionRateByRole } = require('../../utils/constants');
...
exports.getWalletBalance = asyncHandler(async (req, res) => {
  ...
  const [thisMonthEarnings, lastMonthEarnings, thisMonthWithdrawals, totalTransactions] = await Promise.all([
    ...
    WalletTransaction.countDocuments({ userId: nurseObjectId, userType: 'nurse' }),
  ]);
...
// GET /api/nurses/wallet/earnings
exports.getEarnings = asyncHandler(async (req, res) => {
  ...
  const { filter, dateFrom, dateTo } = req.query;
  ...
  // Filter for earnings list (with optional date range or filter type)
  const earningsFilter = { ...baseFilter };
  
  // Apply filter type (today, month, year, all)
  if (filter === 'today') {
    earningsFilter.createdAt = { $gte: today, $lt: tomorrow };
  } else if (filter === 'month') {
    earningsFilter.createdAt = { $gte: currentMonthStart };
  } else if (filter === 'year') {
    earningsFilter.createdAt = { $gte: currentYearStart, $lte: currentYearEnd };
  } else if (dateFrom || dateTo) {
    earningsFilter.createdAt = {};
    if (dateFrom) {
      const start = new Date(dateFrom);
      start.setHours(0, 0, 0, 0);
      earningsFilter.createdAt.$gte = start;
    }
    if (dateTo) {
      const end = new Date(dateTo);
      end.setHours(23, 59, 59, 999);
      earningsFilter.createdAt.$lte = end;
    }
  }
  ...
  const [
    ...
    WalletTransaction.find(earningsFilter)
      .populate('appointmentId', 'appointmentDate fee')
      .populate('orderId', 'totalAmount status')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit),
    WalletTransaction.countDocuments(earningsFilter),
...
// GET /api/nurses/wallet/withdrawals
...
// POST /api/nurses/wallet/withdraw
exports.requestWithdrawal = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await createWalletNotification({
      ...
      withdrawal: withdrawalRequest,
    }).catch((error) => console.error('Error creating nurse withdrawal notification:', error));
    ...
    for (const admin of admins) {
      await createAdminNotification({
        ...
        data: {
          ...
          providerId: id,
          providerType: 'nurse',
          providerName: nurseName,
          providerEmail: nurse.email,
...
// GET /api/nurses/wallet/transactions
